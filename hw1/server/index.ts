...
router
    ...
    .get("/keyWord/:keyWord",
         (ctx, next) => {
           ...
           for (let docIdx of docSet) {
             let docTokenInfos =
                 tokenInfos.filter((tokenInfo) => tokenInfo.docIdx == docIdx);
    ...
    .get("/doc/:docIdx",
         (ctx,
          next) => { ctx.body = JSON.stringify(docs[ctx.params["docIdx"]]); });
app;
