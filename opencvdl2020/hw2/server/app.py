...
@app.route('/q1/<filename>')
def q1_img(filename):
    return send_from_directory("static", "q1/{}".format(filename))


@app.route("/q1-inf")
def q1_inf():
    return jsonify({
        "coin01": len(q1.contours1),
        "coin02": len(q1.contours2),
    })


@app.route('/q2/<filename>')
def q2_img(filename):
    return send_from_directory("static", "q2/{}".format(filename))


@app.route('/q2-intrinsic')
def q2_intrinsic():
    return jsonify({
        "out": q23.q2_mtx.tolist()
    })


@app.route('/q2-extrinsic', methods=['POST'])
def q2_extrinsic():
    req = request.get_json()
    return jsonify({
        "out": np.concatenate(
            [cv2.Rodrigues(q23.q2_rvecs[req["idx"]])[0], q23.q2_tvecs[req["idx"]]], 1).tolist(),
    })


@app.route('/q2-distortion')
def q2_distortion():
    return jsonify({
        "out":  q23.q2_dist.tolist()
    })


@app.route('/q3/<filename>')
def q3_img(filename):
    return send_from_directory("static", "q3/{}".format(filename))


@app.route('/q4-img')
def q4_img():
    with open("./static/q4/disparity.png", 'rb') as bites:
        return send_file(
...
@app.route("/q5-rs", methods=['POST'])
def q5_re():
    img_inf = request.get_json()
    img = cv2.imread(
        "../kagglecatsanddogs_3367a/PetImages/{}/{}.jpg".format(img_inf["class"], img_inf["idx"]))
    img = q5.random_resize(img, 1, 0.4, 0.1, 0.4, 0.1)
    cv2.imwrite("./static/q5/rs.jpg", img)
    with open("./static/q5/rs.jpg", 'rb') as bites:
        return send_file(
            io.BytesIO(bites.read()),
            mimetype='image/jpg'
        )
