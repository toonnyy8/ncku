...
// @ts-ignore
import RERecordSrc from "../tensorboard/random_erasing.png"
...
export const runQ5 = () => {
    ...
    }
    {
        ...
        document.body.appendChild(img)
        let button = document.createElement("button")
        button.innerText = "Random Erasing"
        button.classList.add("lg")
        document.body.appendChild(button)
        button.onclick = () => {
            fetch("http://localhost:5000/q5-re", {
                body: JSON.stringify({ class: imgClass, idx: imgIdx }),
                method: "POST",
                cache: "no-cache", // *default, no-cache, reload, force-cache, only-if-cached
                credentials: "same-origin", // include, same-origin, *omit
    ...
    }
    {
        const data = [
            ...
            { class: "After Random-Erasing", acc: 0.8109622001647949 },
        ]
