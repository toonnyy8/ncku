...
; (async () => {
     ...
     * @type {HTMLCanvasElement}
     */
    const img1Canvas = document.getElementById("img1")
    ...
    let lines = []
    let down = false
    img1Canvas.onmousedown = (e) => {
        down = true
        start_xy = { x: e.x, y: e.y }
    }
    img1Canvas.onmouseup = (e) => {
        down = false
        lines.push([start_xy, { x: e.x, y: e.y }])
    }
    img1Canvas.onmousemove = (e) => {
        if (down) {

            img1Ctx.clearRect(0, 0, 800, 600)
            ...
            lines.forEach((line) => {
                ...
                img1Ctx.lineTo(line[1].x, line[1].y)
                img1Ctx.stroke();
            })
