...
\begin{CJK}{UTF8}{bkai}
   ...
   \begin{tikzpicture}
      \begin{axis}
         \addplot table [x=epoch, y=Test SISNR, col sep=comma, mark=none] {history.csv};
         \addplot table [y=Train SISNR, col sep=comma, mark=none] {../experiment/Normal/history.csv};
      \end{axis}
   \end{tikzpicture}
   % \begin{tikzpicture}
   %    \begin{axis}[
   ...
   %       %    mark=halfcircle*,
   %       %    mark size=2.9pt]
   %       % table[meta=TrainSISNR]
   %       %    {history.csv};
   %       \addplot[red, mark=x] table [y={disppos}]{pistonkinetics.dat}
   %    \end{axis}
   % \end{tikzpicture}
